...
x=x[x$children<5,]
```
We see that in the case of children they usually stay between 0-2 with 3 being 
rare but we have the out-liner of 10 so we deal with it the same way that we did with adults
...
We will analize the same categories but by country insted of hotel grouping first portuguese and non-portuguese

```{r generate_p_np}
x <- mutate(x, portuguese = case_when(country == "PRT" ~ "PRT",
                                         country != "PRT" ~ "NPRT"))

```

```{r hist_adr_tipo_country}
# require(ggplot2)
ggplot(data=x, aes(x=adr, fill=portuguese)) + 
  geom_histogram(bins=50, colour="black") +
  theme_light()
```

```{r violin_adr_tipo_country}
ggplot(data=x, aes(x=portuguese, y=adr, fill=portuguese)) + 
  geom_violin() + geom_boxplot(width=.1, outliers = F) +
  coord_flip() + 
  theme_light()
```
...
# require(ggstatsplot)
ggbetweenstats(data=x, x=portuguese, y=adr)
```

```{r country_2}
# require(tidyverse)
# countries with at least 100 bookings
xx = x %>% group_by(portuguese) %>% mutate(pais=n()) %>% filter(pais>=100)
xx$portuguese=factor(xx$portuguese)
ggplot(data=xx, aes(x=reorder(portuguese, -pais))) + 
  geom_bar(stat="count", aes(fill=hotel)) +
  theme_light() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```

After all this we can se that non portuguese tend to spend a little bit more but
can be explainded beacuse they tend to go to city hotels more and as we have seen
...
Yes, I have chosen a Stacked bar chart, we can steel see the volumes difference
but it's easier to differentiate bettwen canceled and non canceled
```{r alternative}
...
```{r mosaic_country_is_canceled_2}
library(dplyr)

xx2 <- xx %>%
  group_by(country) %>%
  mutate(cancel_rate = mean(is_canceled == 1)) %>%
  ungroup() %>%
  mutate(country = reorder(country, cancel_rate))
ggplot(xx2, aes(x = country, fill = is_canceled)) +
...
```
Yes we can make a 3 categorical variable plot using a heat map where the color 
of the line represents the proportion, but at more complexity the clarity of the
graph gets compromised
...
ggplot(xx3, aes(x = hotel, y = country, fill = prop)) +
  geom_tile() +
...
ggplot(
  data = x,
  aes(x = dia, fill = as.factor(arrival_date_year))
) +
  geom_bar() +
  facet_wrap(~ hotel, ncol = 1) +
  scale_fill_manual(
    values = c("2015" = "red", "2016" = "green", "2017" = "blue"),
    name = "Year"
  ) +
  labs(
    title = "Daily Arrivals by Hotel Type and Year",
    x = "Arrival Date",
...
```
 In country we put the top five in order to have a manageble representation
```{r dia_country}
# require(lubridate)
top_countries <- x %>%
...
ggplot(
  data = x %>% filter(country %in% top_countries),
  aes(x = dia, fill = as.factor(arrival_date_year))
) +
  geom_bar() +
  ...
  labs(
    title = "Daily Arrivals by Country of Origin (Top 5)",
    x = "Arrival Date",
    y = "Number of Bookings"
  ) +
  theme_light() +
...
write.csv(x,"./filename.csv", row.names = FALSE)
```
